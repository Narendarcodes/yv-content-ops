    ...
    <div className="fade-in space-y-8">
      ...
      <section className="card p-0">
        <header className="flex items-center justify-between border-b border-line px-5 py-4">
          ...
          <span className="font-mono text-[11px] text-umber/60">{scheduled.length} upcoming</span>
        </header>
            ...
            return (
              <li key={p.id} className="flex flex-wrap items-center gap-4 px-5 py-4">
                <div className="min-w-0 flex-1">
                  <Link to={`/projects/${p.id}`} className="text-sm font-medium text-ink hover:text-teal">{p.title}</Link>
                  <p className="text-xs text-umber">{[p.platform, p.approvedVersion].filter(Boolean).join(' \u00b7 ') || 'No cut yet'}</p>
                </div>
                <div className="text-right">
                  <p className="font-mono text-xs text-ink">{p.scheduleDate ?? p.publishedAt ?? '-'}</p>
                  <Chip label={p.status === 'PUBLISHED' ? 'Published' : 'Scheduled'} tone={p.status === 'PUBLISHED' ? 'success' : 'warning'} />
                </div>
              </li>
            )
          })}
          {!scheduled.length && <li className="px-5 py-8 text-center text-sm text-umber">Nothing scheduled yet.</li>}
        </ul>
      </section>
